...
import com.example.searchgitapp.Model.ItemIssue;
import com.example.searchgitapp.Model.ItemLabel;
import com.example.searchgitapp.R;

public class IssueAdapter extends RecyclerView.Adapter<IssueAdapter.RespoViewHolder> {

    GithubIssues repList ;
    private Context context;
    private int nums;


    public IssueAdapter(GithubIssues repList, Context context, int nums) {
    ...
    public void onBindViewHolder(@NonNull RespoViewHolder holder, final int position) {
        ...
        ItemIssue item = repList.getItems().get(position);

        holder.tvDescription.setText(item.getState());
        holder.tvOwner.setText(item.getAuthorAssociation());
